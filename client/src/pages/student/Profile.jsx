...
const Profile = () => {
  ...
  const [updateUser, { isLoading: updateUserIsLoading, isError, error, isSuccess }] =
  ...
  useEffect(() => {
    ...
    if (isError) {
      toast.error(error?.message || "Error in updating Profile -CM");
    }
  }, [isSuccess, isError, updateUser, error]);
  ...
  return (
    <div className="mt-4 max-w-4xl mx-auto px-4 md:px-2">
      <h1 className="font-bold text-2xl text-center md:text-left">My Profile</h1>

      <div className="flex flex-col md:flex-row items-center md:items-center gap-8 my-4">
        <div className="flex flex-col items-center">
          <Avatar className="h-24 w-24 rounded-full md:h-32 md:w-32 mb-4">
            <AvatarImage
            ...
            <DialogContent>
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input onChange={onChangeHandler} type="file" accept="image/*" className="col-span-3 border-2 border-black" />
                </div>
              </div>
              <DialogFooter>
