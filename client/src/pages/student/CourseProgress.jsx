...
const CourseProgress = () => {
  ...
  // ✅ Correct isLectureCompleted function
  const isLectureCompleted = (lectureId) => {
    return progress?.some((prog) => prog.lectureId === lectureId && prog.viewed) || completedLectures.includes(lectureId);
  };

  // ✅ Handle lecture progress update and state sync
  const handleLectureProgress = async (lectureId) => {
    try {
      await updateLectureProgress({ courseId, lectureId });
      setCompletedLectures((prev) => [...new Set([...prev, lectureId])]);
      refetch(); // To update the backend state
    } catch (error) {
  ...
  const handeleCompleteCourse = async () => {
    await completeCourse(courseId)
  }
  const handeleInCompleteCourse = async () => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 mt-4">
      ...
      <div className="flex justify-between mb-4">
        ...
        <Button onClick={completed ? handeleInCompleteCourse : handeleCompleteCourse} variant={completed ? "outline" : "default"}>
          {
            completed ? <div className="flex items-center gap-2"><CheckCircle className="h-4 w-4"/> <span>Completed</span></div> : <div><span>Mark as Completed</span></div> 
          }
        </Button>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 md:w-3/5 h-fit rounded-lg shadow-lg p-4">
          ...
          <div className="mt-2">
            <h3 className="font-medium text-lg">
              ...
              ) + 1 || "Not Found"} : ${currentLecture?.lectureTitle || initialLecture?.lectureTitle || "No Lecture Available"
                }`}
            </h3>
          </div>
        </div>
        ...
        {/* Lecture sidebar */}
        <div className="flex flex-col w-full md:w-2/5 border-t md:border-t-0 md:border-l p-4">
          <h2 className="font-semibold text-xl mb-4">Course Lecture</h2>
          <div className="flex-1 overflow-y-auto">
            {courseDetails?.lectures.map((lecture) => (
              <Card
                key={lecture._id}
                className={`mb-3 hover:cursor-pointer transition ${lecture._id === currentLecture?._id ? "bg-green-200 dark:dark:bg-gray-800" : ""
                  }`}
                onClick={() => handleSelectLecture(lecture)}
              >
                <CardContent className="flex items-center justify-between p-1 px-4">
